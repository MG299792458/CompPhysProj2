...
weights, observ = dish.weights, dish.gyration
...
axx.set_ylabel(r"$\langle r_g^2(L) \rangle$")
...
plt.title("Radius of Gyration Rosenbluth\n"+r"$\tilde{\mathcal{C}}=$"+"{:.2f}".format(corr_metric)+"\t"+r"$N=$"+"{}".format(800)+"\t"+r"$L=$"+"{}".format(length))
plt.gcf().set_size_inches(8,5)
ax.set_xlabel(r"polymer length L [$-$]")
plt.savefig("Figures/gyration_R.pdf")
plt.show()